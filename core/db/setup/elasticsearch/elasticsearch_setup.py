# core/db/setup/elasticsearch/elasticsearch_setup.py
# elasticsearch启动脚本
import socket
import sys
import subprocess
...
def check_elasticsearch_installed_windows():
    ...
    if DEFAULT_WINDOWS_PATH.exists():
        _log.info(f"检测到Elasticsearch安装在默认路径：{DEFAULT_WINDOWS_PATH}")
...
def check_elasticsearch_installed_linux():
    ...
    for path in possible_paths:
        if path.exists():
            _log.info(f"检测到Elasticsearch安装在路径：{path}")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            _log.info(f"通过包管理器检测到Elasticsearch安装路径：{install_path}")
            save_elasticsearch_config(install_path)
            return True
    except Exception as e:
        _log.warning(f"无法通过包管理器检测到Elasticsearch安装路径：{e}")

    _log.error("无法检测到Elasticsearch安装。请检查您的安装状态。")
    sys.exit(1)
...
def save_elasticsearch_config(install_path):
    ...
    _log.info(f"Elasticsearch安装路径已保存到配置文件：{ELASTIC_CONFIG_PATH}")


def is_elasticsearch_running():
    for proc in psutil.process_iter(['pid', 'name']):
        if proc.info['name'] == "elasticsearch":
            _log.info("Elasticsearch正在运行")
            return True
    _log.info("Elasticsearch未运行")
    return False
...
def start_elasticsearch_windows():
    try:
        ...
        _log.info("正在尝试启动Elasticsearch服务...")
        ...
        for _ in range(20):  # 尝试20次
            if is_port_open("127.0.0.1", 9200):
                _log.info("Elasticsearch已成功启动并监听端口9200。")
                return True
            time.sleep(2)

        _log.error("Elasticsearch启动失败，未能在预期端口上监听。")
        return False

    except Exception as e:
        _log.error(f"启动Elasticsearch服务失败：{e}")
        return False
...
def stop_all_elasticsearch_processes():
    _log.info("正在停止所有Elasticsearch相关进程...")
    ...
    for proc in psutil.process_iter(['pid', 'name']):
        try:
            if proc.info['name'] in elasticsearch_related_processes:
                _log.info(f"正在终止进程：{proc.info['name']} (PID: {proc.info['pid']})")
                proc.terminate()
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            _log.warning(f"无法终止进程：{proc.info['name']} (PID: {proc.info['pid']}) - 权限不足或进程已结束。")
            continue

    _log.info("所有Elasticsearch相关进程终止命令已发送。")


def check_elasticsearch_connection():
    try:
        _log.info("正在测试Elasticsearch是否已启动...")
        # 通过ping本地端口检查服务是否启动
        if is_port_open("127.0.0.1", 9200):
            _log.info("Elasticsearch已经成功启动并在9200端口监听。")
            return True
        else:
            _log.error("Elasticsearch未能在9200端口启动。")
            return False
    except Exception as e:
        _log.error(f"检查Elasticsearch连接时发生错误：{e}")
        return False


if __name__ == "__main__":
    _log.info("开始Elasticsearch启动检测...")

    # 添加连接测试
    if check_elasticsearch_connection():
        _log.info("Elasticsearch已经在运行，连接正常。")
        sys.exit(0)

    if not check_elasticsearch_installed():
        _log.error("Elasticsearch未安装或安装检测失败。")
        sys.exit(1)

    if not is_elasticsearch_running():
        _log.info("Elasticsearch未运行，尝试启动...")
        stop_all_elasticsearch_processes()
        start_elasticsearch()

    if not check_elasticsearch_connection():
        _log.error("Elasticsearch启动失败或无法连接，请检查安装和配置。")
        sys.exit(1)

    _log.info("Elasticsearch准备就绪。")
    sys.exit(0)
